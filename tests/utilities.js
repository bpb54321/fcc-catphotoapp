...
const colors = require('colors'); // Used for printing messages to console
...
exports.printErrorAndExit = async function(error, driver) {
  console.log(colors.red(error.stack));
  await driver.quit();
  process.exit(1);
}

exports.checkElementText = async function(web_element, text) {
  let web_element_text = await web_element.getText();
  console.log(colors.yellow(`Checking that ${web_element} has correct text...\n`));
    expect(web_element_text).to.equal(text);
    console.log(colors.green(`\t${web_element} has correct text.\n`));
}
